...
const stagingConfig = {
  apiKey: "AIzaSyAFUh28UP69m2nDrijuRT6JWds-9ksk4_U",
  authDomain: "bridge-content-dashboard-3e9e7.firebaseapp.com",
  databaseURL: "https://bridge-content-dashboard-3e9e7.firebaseio.com",
  projectId: "bridge-content-dashboard-3e9e7",
  storageBucket: "bridge-content-dashboard-3e9e7.appspot.com",
  messagingSenderId: "116391050087"
};
...
const config = process.env.REACT_APP_PROD ? prodConfig : stagingConfig;
  ...
  callbacks: {
    signInSuccessWithAuthResult: authResult =>
      reduxStoreInstance.dispatch({
        type: "SIGNIN_SUCCESS_WITH_CREDENTIALS",
        payload: authResult
      })
  },
...
export const modulesFirebase$ = fromEvent(
  (firebase as any).database().ref("/modules") as any,
  "value"
).pipe(
  filter(Boolean),
...
export const updateModuleFirebase = (moduleID, moduleValue) =>
  Observable.create(obs => {
    firebase
      .database()
      .ref(`/modules/${moduleID}`)
      .set(moduleValue, () => obs.next("Success"));
  });
...
export const setCohort = (cohortName, moduleIds, startDate, endDate) => {
  return from(
    ((firebase as any).database().ref(`/cohort`) as any).push({
      cohortName,
      moduleIds,
      startDate,
      endDate
...
fromEvent((firebase as any).database().ref(`/cohort/`) as any, "value")
  .pipe(
    filter(Boolean),
...
export const addClassroomToCohort = (cohortId, classroom) => {
  return Observable.create(obs => {
    ((firebase as any)
      .database()
      .ref(`/cohort/${cohortId}/classrooms`) as any).push(classroom, () => {
      obs.next("success!");
    });
  });
};
...
export const updateClassroomToCohort = (cohortId, classroomId, classroom) => {
  return Observable.create(obs => {
    ((firebase as any)
      .database()
      .ref(`/cohort/${cohortId}/classrooms/${classroomId}`) as any).update(
      classroom,
      () => {
